...
def run_nist_tests(filename="ciphertext.bin"):
    # Run NIST Statistical Test Suite
    result = subprocess.run(['nist_sts', filename], capture_output=True, text=True)
...
# Save a ciphertext to file for NIST tests
save_ciphertext_to_file(ciphertexts[0], "ciphertext.bin")

# Run NIST tests on the saved ciphertext file
run_nist_tests("ciphertext.bin")
