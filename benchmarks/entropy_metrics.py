...
def main():
    ...
    plt.plot(range(len(entropies)), entropies, color='b',
             alpha=0.7, label='Rolling Entropy (1KB Chunks)')
    plt.axhline(y=7.99, color='r', linestyle='--',
                label='Ideal Target (7.99 bits)')
    ...
    plt.bar(range(256), sorted_counts, color='teal',
    ...
    print(
        f"Global Shannon Entropy of 1MB Keystream: {calculate_shannon(ciphertext):.5f} bits/byte")
