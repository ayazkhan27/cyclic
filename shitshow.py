...
def measure_khan_encryption(plaintext, start_position, superposition_sequence_length):
    ...
    modified_plaintext = plaintext[:len(plaintext)//2] + 'A' + plaintext[len(plaintext)//2 + 1:]
    modified_ciphertext, _, _, _, _, _, _, _ = ke.khan_encrypt(modified_plaintext, cyclic_prime, cyclic_sequence, start_position)
    ...
    modified_plaintext = 'A' * (len(plaintext) // 2) + ''.join(random.choice(string.ascii_letters + string.digits) for _ in range(len(plaintext) // 2))
    modified_ciphertext, _, _, _, _, _, _, _ = ke.khan_encrypt(modified_plaintext, cyclic_prime, cyclic_sequence, start_position)
    ...
    return encryption_time, decryption_time, decrypted_text, avalanche_ratio, propagation_ratio, entropy
...
def main():
    ...
    encryption_time, decryption_time, decrypted_text, avalanche_ratio, propagation_ratio, entropy = measure_khan_encryption(plaintext, start_position, superposition_sequence_length)
