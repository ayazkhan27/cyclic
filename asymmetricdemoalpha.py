...
# Calculate entropy (Shannon entropy of ciphertext)
def calculate_entropy(data):
    data_set = list(set(data))
    freq_list = [data.count(entry)/len(data) for entry in data_set]
    entropy = -sum([freq * np.log2(freq) for freq in freq_list])
    return entropy

# Measure avalanche effect (difference between ciphertexts when a single bit is changed)
def measure_avalanche_effect(ciphertext1, ciphertext2):
    diff_bits = sum(bin(c1 ^ c2).count('1') for c1, c2 in zip(ciphertext1, ciphertext2))
    total_bits = len(ciphertext1) * 8  # Each character is 8 bits
...
if __name__ == "__main__":
    ...
    entropy = calculate_entropy(encrypted_msg)
    print(f"Entropy of ciphertext: {entropy:.4f} bits")

    # Measure avalanche effect (compare with slight message modification)
    modified_msg = msg[:-1] + chr((ord(msg[-1]) + 1) % 256)
    encrypted_modified_msg, _, _ = encrypt(modified_msg, public_key)
    avalanche_effect = measure_avalanche_effect(encrypted_msg, encrypted_modified_msg)
    print(f"Avalanche effect (bit difference between original and modified ciphertexts): {avalanche_effect * 100:.2f}%")
