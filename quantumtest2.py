...
def measure_khan_encryption(plaintext, start_position, superposition_sequence_length):
    ...
    return encryption_time, decryption_time, decrypted_text, ciphertext, char_to_movement, movement_to_char, z_value, superposition_sequence, iv, salt, z_layers, cyclic_sequence
...
def main():
    global cyclic_prime
    cyclic_prime = 32779  # Set the cyclic prime to 1051
    ...
    encryption_time, decryption_time, decrypted_text, ciphertext, char_to_movement, movement_to_char, z_value, superposition_sequence, iv, salt, z_layers, cyclic_sequence = measure_khan_encryption(plaintext, start_position, superposition_sequence_length)
    ...
    with ThreadPoolExecutor() as executor:
        ...
        try:
            decrypted_text, found_key = future.result(timeout=timeout_seconds)
            if decrypted_text:
                print(f"Decryption Successful: {decrypted_text}")
            else:
                print("Grover's attack simulation failed to decrypt the message.")
        except TimeoutError:
            print("Grover's attack simulation timed out before completing.")
