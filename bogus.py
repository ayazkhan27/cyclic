...
# Your existing encryption and analysis functions
...
# Test Information-Rich vs Random Noise
def test_entropy():
    prime = 17  # Example prime, replace with your full reptend prime
    cyclic_sequence = find_cyclic_sequence(prime)
    
    # Generate random noise and information-rich message
    random_noise = generate_plaintext(len(cyclic_sequence))
    information_rich_message = cyclic_sequence
    
    # Calculate entropies
    entropy_random_noise = calculate_entropy(random_noise)
    entropy_information_rich = calculate_entropy(information_rich_message)

    print("Random Noise:")
    print(f"Message: {random_noise}")
    print(f"Entropy: {entropy_random_noise}")

    print("\nInformation-Rich Message:")
    print(f"Message: {information_rich_message}")
    print(f"Entropy: {entropy_information_rich}")

    # Use typical entropy threshold to distinguish
    entropy_threshold = 4.5  # Example threshold, adjust based on requirements
    random_noise_classification = "Random Noise" if entropy_random_noise > entropy_threshold else "Information-Rich"
    information_rich_classification = "Random Noise" if entropy_information_rich > entropy_threshold else "Information-Rich"

    print("\nClassification based on Entropy:")
    print(f"Random Noise classified as: {random_noise_classification}")
    print(f"Information-Rich Message classified as: {information_rich_classification}")

    # Use your algorithm to distinguish
    your_algorithm_random_noise_classification = "Information-Rich" if check_khan_theorems(random_noise) else "Random Noise"
    your_algorithm_information_rich_classification = "Information-Rich" if check_khan_theorems(information_rich_message) else "Random Noise"

    print("\nClassification based on Your Algorithm:")
    print(f"Random Noise classified as: {your_algorithm_random_noise_classification}")
    print(f"Information-Rich Message classified as: {your_algorithm_information_rich_classification}")
